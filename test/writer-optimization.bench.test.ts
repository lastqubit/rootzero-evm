...
const methods = [
  ...["Block", "Step", "Call", "Dispatch", "Relay", "Context", "Recover", "Label", "Schema"].map(name => ({ name, method: "append" + name })),
  ...["Block", "List", "Bytes", "String", "Step", "Call", "Dispatch", "Relay", "Context", "Recover"].map(name => ({ name, method: "copy" + name })),
];
...
describe("Writer reserved length reuse", function () {
  this.timeout(180_000);
  it("compares every output layout across buffer growth and preallocated capacity", async () => {
    const helper = await deploy("TestWriterOptimization");
    const rows: any[] = [];
    ...
    writeFileSync(".npm-cache/writer-results.json", JSON.stringify(rows, null, 2) + "\n");
  ...
  it("handles empty first or second payloads at nonzero output positions", async () => {
    ...
    for (const name of ["Relay", "Context"]) {
      for (const method of ["append" + name, "copy" + name]) {
