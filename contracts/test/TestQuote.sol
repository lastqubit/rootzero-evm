...
contract TestQuote {
    ...
    function create(Quote memory quote) external pure returns (bytes memory) {
        return Blocks.createQuote(quote.asset, quote.amount, quote.liability, quote.debt);
    }
    ...
    function write(Quote memory quote, bool scalar) external pure returns (bytes memory) {
        ...
        if (scalar) writer.appendQuote(quote.asset, quote.amount, quote.liability, quote.debt);
    ...
    function decode(bytes calldata input, bool scalar) external pure returns (Quote memory quote) {
        ...
        if (scalar) (quote.asset, quote.amount, quote.liability, quote.debt) = cur.unpackQuote();
