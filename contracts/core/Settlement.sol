...
abstract contract Settlement is HostAccount, DebitAccountHook, CreditAccountHook, BookHook, SettleHook {
    ...
    /// @notice Apply the final position quantities exactly; callers enforce limits.
    ...
    function settle(bytes32 account, Position memory position) internal virtual override {
        bytes32 counterparty = position.counterparty;
        if (counterparty == bytes32(0)) {
            book(account, account, position.asset, position.amount, position.liability, position.debt);
            return;
        }
        if (position.debt != 0) {
            book(account, counterparty, position.liability, position.debt, position.liability, position.debt);
        }
        if (position.amount != 0) {
            book(counterparty, account, position.asset, position.amount, position.asset, position.amount);
        }
    }
}
